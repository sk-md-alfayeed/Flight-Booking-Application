...
public class FareManagementServiceImpl implements FareManagementService {
	...
	public String addFare(Fare fare) {
		...
		// Producer
		rabbitTemplate.convertAndSend(RabbitMQConfig.EXCHANGE, RabbitMQConfig.POST_ROUTING_KEY, fare);
...
//		fareManagementRepository.save(fare);
	...
	public String updateFare(Fare fare) {
		...
		// Producer
		rabbitTemplate.convertAndSend(RabbitMQConfig.EXCHANGE, RabbitMQConfig.PUT_ROUTING_KEY, fare);
...
//		fareManagementRepository.save(fare);
		try {
			...
			try {
				flight.setFare(fare);
				String tokenUpdateFlight = restTemplate.postForObject("http://flight-management/flight/authenticate",
						auth, String.class);
